...
execSync(`pipenv run python ./firebase/firestore.py add "${songA.artist}" "${songA.title}" "${songA.file}"`, {
  stdio: "inherit",
  shell: true,
});
...
execSync(`pipenv run python ./firebase/firestore.py retrieve "${songA.artist}" "${songA.title}"`, {
  stdio: "inherit",
  shell: true,
});
